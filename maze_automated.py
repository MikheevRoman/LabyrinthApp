...
QUACKS_FILEPATH = "/Users/andrey/PycharmProjects/nir/duck_sounds"
        ...
        while( self._running ):
            ...
            if self.turn == self.num_moves or max(player.speed for player in self.players)==0:
                ...
                if self.made_it_proportion > MADEIT_THRESH:
                    print("{}% HAVE MADE IT, BLOODY WELL DONE LEGENDS, on generation {}".format(self.made_it_proportion*100, self.generations))
                    self.on_cleanup()
                    break
...
class Maze:
    def __init__(self):
        ...
        self.maze = list(list(0 for j in range(self.M)) for i in range(self.N))
        temp_maze = perfect_maze.growing_tree(self.M, self.N)
        ...
        new_shape = (1, self.M * self.N)
        self.maze = self.maze.reshape(new_shape)[0]
